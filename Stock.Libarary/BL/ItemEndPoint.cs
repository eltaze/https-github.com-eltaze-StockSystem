...
namespace StockSystem.Libarary.BL
{
    public class ItemEndPoint : Reposit<Item>, IItemEndPoint
    {
        ...
        public override void Save(Item t)
        {
            ...
            try
            {
                sql.Execute<Item, dynamic>("spItemInsert", new { Name=t.Name,KindId=t.kindId,Note=t.Note,UnitId=t.UnitId,departmentid=t.DepartmentId,barcode=t.Barcode, DisplayInDash =t.DisplayInDash});
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
